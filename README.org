...
** Explanation
Critically, this file and my entire emacs configuration does not live in my =.emacs.d= directory. Both it, =init.el=, and =early-init.el= are all symlinked to that directory because I much prefer to host my emacs configuration in cloud-synced directory. That means that whenever I make changes to this file, I can simply restart the emacs daemon on my other machines to have the up-to-date configuration.

The alternative to this of course is to simply use git and push and pull changes. But that would be more inconvenience than I want to incur for simple configuration.
*** References
...
*** Customization & Usage
If you would like to use this configuration, then edit the contents of the file =.personal-example.el= and rename it to =.personal.el=. Then symbolically link it to your home directory. Either use the following command in a terminal, or use =dired=, where the key to create symbolic links is "S".
#+begin_src shell
...
(use-package dirvish
  ...
  (setq dirvish-cache-dir my-dirvish-cache-dir)
...
(use-package no-littering
  :ensure t
  :init
  (setq no-littering-etc-directory (expand-file-name (concat my-emacs-path "config"))
        no-littering-var-directory (expand-file-name (concat my-emacs-path "data"))
...
(use-package vterm
  ...
  (corfu-mode)
  :init
  ;; (load-file "~/Nextcloud/E/emacs/artifacts/client-side-vterm.el")
...
(setq shell-file-name my-shell
      vterm-tramp-shells `(("docker" ,(format "/usr/bin/env %s" my-shell)))
      vterm-shell (format "/run/current-system/sw/bin/%s" my-shell)
...
**** org
#+begin_src emacs-lisp
(load-file "~/Nextcloud/E/emacs/artifacts/org-buffer-title-instead-of-filename.el")
#+end_src
*** bookmarks
#+begin_src emacs-lisp
...
(use-package bufferlo
  ...
  :config
  (bufferlo-mode 1))
(load-file (expand-file-name "homegrown/my-consult-buffer-sources.el" my-emacs-path))
(setq consult-buffer-sources '(my/consult--source-file-register
                               my/consult--source-hidden-buffer
                               my/consult--source-recent-file
                               my/consult--source-modified-buffer
                               my/consult--source-bookmark
...
(use-package embark-consult
  ...
  :ensure t)
#+end_src
**** [[https://github.com/emacs-mirror/emacs/blob/master/lisp/savehist.el][Savehist]]
...
(use-package corfu
  :straight (corfu 
             ...
             :repo "minad/corfu")
  :init
  ;; (load-directory "~/.emacs.d/straight/repos/corfu/extensions/")
  (corfu-history-mode)
...
(use-package pdf-view-restore
  ...
  :custom
  (pdf-view-restore-filename (expand-file-name "data/.pdf-view-restore" my-emacs-path)))
#+end_src
*** calc
**** Literate calc mode
#+begin_src emacs-lisp
...
(setq org-web-tools--pandoc-no-wrap-option t)
...
**** Select font
#+begin_src emacs-lisp
(set-face-attribute 'org-modern-symbol nil :family my-selected-font :height 120)
#+end_src
...
(defun my/writing-mode ()
  "'olivetti-mode', 'flyspell-mode', 'org-modern-mode'."
  (interactive)
  (olivetti-mode)
  (flyspell-mode))
...
(defun my/toggle-keybindings ()
  "Lucian's default bindings keymap."
  ;; God mode
  (global-set-key (kbd "<escape>") #'god-mode-all)
  ;; Window
  (my/bind-key-dwim "C-x O" 'window-swap-states)
  (dolist (cmd '(window-swap-states))
  ...
  (defun my/find-from-home (&optional initial)
    "Find file with ~/users/home/<user> as initial directory"
    (interactive)
    (affe-find gnus-home-directory))
  (my/bind-key-dwim "C-c f" 'my/find-from-home)
  ;; org mode

...
(use-package magit
  ...
  :init
  ;; https://github.com/magit/magit/discussions/4973#discussioncomment-8858908
  ...
  (defun sanityinc/on-eat-exit (process)
    (when (zerop (process-exit-status process))
      ...
      (unless (eq (selected-window) (next-window))
        (delete-window))))
  (defun my/eat-dwim ()
    "Always open new session, and at root if inside project."
    (interactive)
    (if (project-current)
        (eat-project t)
        (eat nil t)))
...
(use-package gptel
  ...
  (setq gptel-directives
        '((default . "")
          (elisp-hacker . "You are an expert emacs lisp programmer and have written several widely used packages, admired for their utility despite their simplicity. Comments should be prepended with ;;")
          (python-programmer . "You are an expert python programmer, and do things the pythonic way. Anything you respond with that is not code should be commented. Respond concisely."))))
#+end_src
...
(use-package dape
  :ensure t)
#+end_src
**** [[https://github.com/jdtsmith/eglot-booster][GitHub - jdtsmith/eglot-booster: Boost eglot using lsp-booster]]
#+begin_src emacs-lisp
...
(defun transpose-dwim (&optional ARG)
  ...
  (let ((universal    (or ARG 1))
    ...
    (cond ((and (= right-char ? ) (= left-char ?.)) (transpose-sentences universal))
          ((= right-char ?\n) (transpose-lines universal))
